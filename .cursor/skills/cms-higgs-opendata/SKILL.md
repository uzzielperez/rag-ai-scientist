...
2. Indexable narrative: **`docs/examples/cms_higgs_opendata_physics_story.md`**. PyPI-only installs should copy that file into their references directory or extend **`configs/references.yaml`**, then **`rag-ai-scientist setup-rag --project-root ...`**.
...
- **`get_skill`** with `skill=cms-higgs-opendata` works after **`pip install rag-ai-scientist`** (packaged copy under **`rag_ai_scientist/skills/`**). This file overrides when present in **`.cursor/skills/`**.
- **`query_analysis_knowledge`** requires **`setup-rag`** so **`.cursor/rag_db`** exists for the chosen **`--project-root`**.
